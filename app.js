...
const getAllPost = () =>{
    ...
    let posts = readlsData('fb-post');
    let list='';

    if(!posts){
        allPosts.innerHTML = '<div class="card shadow"><div class="card-body text-center">No Post Found</div></div>';
        return false;
    }

    posts.reverse().map((data, index)=>{
        list +=`
            <div class="fbTimeline my-4">
                    ...
                    <div class="postAuthArea">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-white" aria-labelledby="dropdownMenuLink">
                                <li><a class="dropdown-item editPost" post_id=${data.id} data-bs-toggle="modal" href="#post-edit-modal">Edit</a></li>
...
postAddform.onsubmit=(e)=>{
    ...
    const ranId = Math.floor(Math.random()*1000000)+'_'+Date.now();
    ...
    if( !aname || !aphoto || !pcontent ){
        ...
        createLsData('fb-post', {...data, id: ranId });
        e.target.reset();
        getAllPost();
    };
...
allPosts.onclick=(e)=>{
    ...
    if(e.target.classList.contains('editPost')){
        ...
        // get all post
        const allData = readlsData('fb-post');
        
        // get edit single post data from array
        const editPost = allData.find(tomi => tomi.id == postId);
        // get single post data key
        const {aname, aphoto, pphoto, pcontent}= editPost;

        postEditform.innerHTML =`

            <div class="my-3">
            <input name="index" value="${postId}" class="form-control" type="text">
            </div>

            <div class="my-3">
            <label for="">Auth Name</label>
            <input name="aname" value="${aname}" class="form-control" type="text">
            </div>
            ...
            <div class="my-3">
                ...
                <input name="aphoto" value="${aphoto}" class="form-control" type="text">
            </div>

            <div class="my-3">
                <label for="">Post Content</label>
                <input name="pcontent" value="${pcontent}" class="form-control" type="text">
            </div>

            <div class="my-3">
                    <img width="80" src="${pphoto}" alt="">
                </div>

            <div class="my-3">
                <label for="">Post Photo</label>
                <input name="pphoto" value="${pphoto}" class="form-control" type="text">
            </div>

            <div class="my-3">
                <input value="Edit Post" class="btn btn-primary w-100" type="submit">
            </div>
        `
    };
    ...
    if(e.target.classList.contains('deletePost')){
        ...
        updatelsData('fb-post', deleteData);
